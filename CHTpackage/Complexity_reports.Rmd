...
```{r}
best_gs_bucket_fast <- function(men_prefs, women_prefs) {
...
test_gs_bucket_time <- function(n) {
  ...
  time <- system.time({
    best_gs_bucket_fast(men_prefs, women_prefs)
  })
  return(time["elapsed"])
}
```
...
ggplot(df, aes(x = n)) +
  ...
  geom_line(aes(y = n_squared), linetype = "dashed", size = 1, color = 'green') +
  labs(
    title = "Temporal Complexity of Gale–Shapley bucket (Empirical vs n²)",
    x = "n (number of men/women)",
    y = "Execution time (seconds)"
  ) +
  theme_minimal(base_size = 14)
...
```{r}
times_r   <- sapply(sizes, bench_heap_r)
times_cpp <- sapply(sizes, bench_heap_cpp)

df <- data.frame(
  n = rep(sizes, 2),
  time = c(times_r, times_cpp),
  version = rep(c("R Heap", "C++ Heap"), each = length(sizes))
)
```
...
ggplot(df, aes(x = n, y = time, color = version)) +
  ...
  geom_point(size = 2) +
  labs(
    title = "Gale–Shapley Heap: R vs C++ Runtime Comparison",
    x = "Problem size (n)",
...
ggplot(df, aes(x = n, y = time, color = version)) +
  ...
  scale_y_log10() +
  labs(
    title = "Gale–Shapley Heap: R vs C++ (Log–Log Scale)",
    x = "log(n)",
    y = "log(time)"
  ) +
  theme_minimal(base_size = 14)
