...
```{r, example1}
# Example 1 preference lists

men_prefs <- list(
  Camille = c("Bérénice", "Prudence", "Lila", "Sarah", "Elodie"),
  Gérard  = c("Sarah", "Elodie", "Lila", "Prudence", "Bérénice"),
  Thibault = c("Lila", "Bérénice", "Elodie", "Sarah", "Prudence"),
  Sacha   = c("Elodie", "Lila", "Prudence", "Bérénice", "Sarah"),
  Jérôme  = c("Prudence", "Sarah", "Elodie", "Lila", "Bérénice")
)

women_prefs <- list(
  Bérénice = c("Gérard", "Camille", "Sacha", "Thibault", "Jérôme"),
  Prudence = c("Sacha", "Jérôme", "Camille", "Gérard", "Thibault"),
  Lila     = c("Camille", "Thibault", "Jérôme", "Sacha", "Gérard"),
  Sarah    = c("Thibault", "Sacha", "Gérard", "Jérôme", "Camille"),
  Elodie   = c("Jérôme", "Camille", "Thibault", "Gérard", "Sacha")
)
```
...
men_prefs_1 <- list(
  Camille  = c("Bérénice", "Prudence", "Lila", "Sarah", "Elodie"),
  Gérard   = c("Bérénice", "Prudence", "Lila", "Sarah", "Elodie"),
  Thibault = c("Bérénice", "Prudence", "Lila", "Sarah", "Elodie"),
  Sacha    = c("Bérénice", "Prudence", "Lila", "Sarah", "Elodie"),
  Jérôme   = c("Bérénice", "Prudence", "Lila", "Sarah", "Elodie")
)

women_prefs_1 <- list(
  Bérénice = c("Jérôme", "Sacha", "Thibault", "Gérard", "Camille"),
  Prudence = c("Jérôme", "Sacha", "Thibault", "Gérard", "Camille"),
  Lila     = c("Jérôme", "Sacha", "Thibault", "Gérard", "Camille"),
  Sarah    = c("Jérôme", "Sacha", "Thibault", "Gérard", "Camille"),
  Elodie   = c("Jérôme", "Sacha", "Thibault", "Gérard", "Camille")
)
```
...
```{r, algo1-2}}
result_gs_1 <- gale_shapley(men_prefs_1, women_prefs_1)
result_gs_1
```
...
```{r, algo2-2}
result_bucket_1 <- best_gs_bucket(men_prefs_1, women_prefs_1)
result_bucket_1
```

Algorithm3 - Hopcroft-Karp for Blood Donor Matching
```{r, algo3}
# Example blood type data
data <- data.frame(
  id = 1:6,
  blood_type = c("A+", "O-", "B+", "AB+", "AB-", "O+")
)
donors <- c(1, 2, 3)      # IDs of donors
receivers <- c(4, 5, 6)   # IDs of receivers
compatibility_table <- create_compatibility_table()
result_hk <- hopcroft_karp(donors, receivers, data, compatibility_table, can_receive)
print_matching_results(result_hk, donors, receivers, data)
```
# Complexity
...
```{r, gs-bucket-complexity}
best_gs_bucket <- function(men_prefs, women_prefs) {

  men_names   <- names(men_prefs)    # O(n)
  women_names <- names(women_prefs)       # O(n)

  n <- length(men_prefs)

  men_index   <- setNames(seq_len(n), men_names)     # O(n)
  women_index <- setNames(seq_len(n), women_names)    # O(n)

  men_pref_num <- vector("list", n)      # O(n)
  for (h in seq_len(n)) {
    men_pref_num[[h]] <- women_index[ men_prefs[[h]] ] # O(n)   (each preference list has length n)
  }

  women_pref_num <- vector("list", n)        # O(n)
  for (f in seq_len(n)) {
    women_pref_num[[f]] <- men_index[ women_prefs[[f]] ] # O(n)
  }

  women_rank <- vector("list", n)        # O(n)
  for (f in seq_len(n)) {
    pref <- women_pref_num[[f]]       # O(n)
    r <- integer(n)            # O(n)
    for (pos in seq_len(n)) {        # Loop: n iterations
      r[ pref[pos] ] <- pos        # O(1) chacun donc O(n)
    }
    women_rank[[f]] <- r              # O(1)
  }
  # total building women_rank: O(n²)

  next_choice <- rep(1L, n)    # O(n)
  matching    <- rep(NA_integer_, n)        # O(n)

  buckets <- vector("list", n)       # O(n)

  for (h in seq_len(n)) {
    f <- men_pref_num[[h]][1]          # O(1)
    buckets[[1]] <- append(buckets[[1]], list(c(h,f))) #O(1)
  }
  while (TRUE) {

    p <- match(TRUE, lengths(buckets) > 0)       # O(n)   (scan all buckets)
    if (is.na(p)) break         # O(1)

    prop <- buckets[[p]][[1]]          # O(1)
    buckets[[p]] <- buckets[[p]][-1]       # O(1)

    h <- prop[1]         # O(1)
    f <- prop[2]         # O(1)

    current <- match(f, matching)       # O(n)   (match scans vector)
    ...
    if (is.na(current)) {
      ...
      rc <- women_rank[[f]][current]      # O(1)
      rn <- women_rank[[f]][h]         # O(1)

      if (rn < rc) {

        matching[h]       <- f          # O(1)
        matching[current] <- NA_integer_     # O(1)

        next_choice[current] <- nc <- next_choice[current] + 1L # O(1)
        if (nc <= n) {
          f2 <- men_pref_num[[current]][nc]           # O(1)
          buckets[[nc]] <- append(buckets[[nc]], list(c(current,f2))) # O(1)
        }

      } else {

        next_choice[h] <- nh <- next_choice[h] + 1L   # O(1)
        if (nh <= n) {
          f2 <- men_pref_num[[h]][nh]                 # O(1)
          buckets[[nh]] <- append(buckets[[nh]], list(c(h,f2))) # O(1)
        }
      }
    }
  }
  ...
  data.frame(
    Man   = men_names,          # O(n)
    Woman = women_names[matching],        # O(n)
    stringsAsFactors = FALSE
  )
}
```

L’algorithme de Gale–Shapley basé sur des buckets effectue au maximum $n^2$ propositions.
Alors que la plupart des updates (recherches de préférences, comparaisons de rang, insertions de bucket) s’exécutent en temps constant, deux opérations à l’intérieur de la boucle principale introduisent un facteur linéaire supplémentaire : le balayage du tableau de bucket pour localiser le premier non-empty bucket, et scanner le vecteur correspondant pour vérifier le partenaire actuel d’une femme.
Puisque ces opérations O(n) se produisent une fois par proposition, le temps d’exécution total devient :

$$C(\text{Gale-Shapley with bucket})=O(n^3)$$
...
sizes <- seq(300, 3000, by = 300)
# Measure runtime for each size
times_bucket <- sapply(sizes, test_gs_bucket_time)

# Build data frame for plotting
df <- data.frame(
  n = sizes,
  time = times_bucket,
  n_squared = (sizes^2) / max(sizes^2) * max(times_bucket),          # scaled n²
  n_cubed  = (sizes^3) / max(sizes^3) * max(times_bucket),
  n_log_n  = (sizes * log(sizes)) / max(sizes * log(sizes)) * max(times_bucket)  # scaled n log n
)
...
ggplot(df, aes(x = n)) +
  ...
  annotate("text", x = 500, y = max(df$time) * 0.5, 
           ...
           color = "#E41A1C", size = 4, fontface = "bold")
```
### Gale-shapley with bucket vs without

Comparons cette nouvelle version avec l'originale :
...
```{r}
# Plot measured time of gs with and without bucket
ggplot(df, aes(x = n)) +
  geom_line(aes(y = time1), size = 1.2, color = 'red') +
  geom_point(aes(y = time1), size = 2, color = 'red') +
  geom_line(aes(y = time2), size = 1.2, color = 'purple') +
  geom_point(aes(y = time2), size = 2, color = 'purple') +
  labs(
    title = "Time Complexity of Gale–Shapley with and without bucket",
    x = "n (number of men/women)",
    y = "Execution time (seconds)"
  ) +
  theme_minimal(base_size = 14) +
  annotate("text", x = 1000, y = max(df$time2) * 0.6, 
           label = print("with bucket"),
           color = "purple", size = 4, fontface = "bold") +
  annotate("text", x = 1000, y = max(df$time2) * 0.4, 
           label = print("without bucket"),
...
```{r, gs-bucket-r-vs-cpp-plot}}


times_r   <- times_bucket
times_cpp <- sapply(sizes, test_gs_bucket_time_cpp)

df <- data.frame(
  n = rep(sizes, 2),
  time = c(times_r, times_cpp),
  version = rep(c("R bucket", "C++ bucket"), each = length(sizes))
)
...
```{r, gs-bucket-r-vs-cpp-loglog-plot}}
ggplot(df, aes(x = n, y = time, color = version)) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  scale_x_log10() +
...
build_compatibility_graph  # O(n^2)

bfs_hopcroft_karp  # O(E)

dfs_hopcroft_karp # O(E)

hopcroft_karp # O(E * sqrt(n))
...
```
On n'observe pas une comlexité comme théorique O(E·√V) car des fonctions annexes et l'implémentation viennent ralentir l'algorithme.

```{r, hopcroft-karp log-log plot}
